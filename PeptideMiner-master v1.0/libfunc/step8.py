...
def analysis():
    ...
    with open('./02-pipeline/step8_output_summary.txt','w') as out:
        ...
        out.write("\n")
        out.write(f"Number of profile-HMMs used: {len(hmms)}\n")
        out.write(f"\t{','.join(hmms)}\n")
        out.write(f"Number of databases searched: {len(queries)}\n")
        out.write(f"\t{','.join(queries)}\n")
        out.write("\n")
        out.write(f"Output:\n")
        out.write(f"hmmsearch hits: {n_hmmsout}\n")
        out.write(f"PeptideMiner hits: {len(PeptideMinerhits)}\n")
        out.write(f"\tNumber of unique CDS: {len(unique_pre)}\n")
        out.write(f"\tNumber of unique mature peptides: {len(unique_matseq)}\n")
